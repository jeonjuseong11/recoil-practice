...
export const sendVerificationCode = async (email: string) => {
  try {
    const response = await instance.post('/send-verification-email', {
      email,
    });
    return response.status;
  } catch (error) {
...
export const verifyEmail = async (code: string, email: string) => {
  try {
    const response = await instance.post('/verify-email', { code, email });
    return response.status;
  } catch (error) {
...
export const updateToken = async (token: string) => {
  try {
    const response = await instance.post('/update-token', { token });
